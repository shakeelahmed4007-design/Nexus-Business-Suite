...
export const authenticateJWT = async (
  ...
  try {
    ...
    let role = (profile?.role as string) || (user.user_metadata?.role as string) || 'staff';
    if (role === 'admin') role = 'shop_admin';
    if (!['super_admin', 'shop_admin', 'staff'].includes(role)) role = 'staff';

    const shopId = profile?.shop_id || user.user_metadata?.shop_id || (req.headers['x-shop-id'] as string) || 'shop-001';
