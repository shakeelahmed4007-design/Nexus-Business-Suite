...
export const createOrder = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ success: false, error: 'items array is required', code: 'BAD_REQUEST' });
    }
    ...
    return res.status(400).json({ success: false, error: err.message, code: 'BAD_REQUEST' });
  }
};
...
export const updateOrderStatus = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!status) {
      return res.status(400).json({ success: false, error: 'status field is required', code: 'BAD_REQUEST' });
    }
