...
export class IntegrationController {
  ...
  /**
   * PATCH /api/integrations/inbox/threads/:threadId/assign
   * Assign message thread to a specific staff/sales agent
   */
  public async assignThread(req: AuthenticatedRequest, res: Response) {
    try {
      const shopId = req.user?.shop_id || 'shop-001';
      const threadId = req.params.threadId;
      const { assigned_to_user_id, assigned_to_name } = req.body;
      await unifiedInboxService.assignThread(shopId, threadId, assigned_to_user_id, assigned_to_name);
      return res.json({ success: true, message: `Thread assigned to ${assigned_to_name || 'agent'}` });
    } catch (err: any) {
      return res.status(500).json({ success: false, error: err.message });
    }
  }
