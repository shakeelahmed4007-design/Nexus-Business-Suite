...
export class UnifiedInboxService {
  ...
  /**
   * Assign message thread to a specific staff/sales team member
   */
  public async assignThread(shopId: string, threadId: string, assignedToUserId: string, assignedToName: string) {
    try {
      await supabaseAdmin
        .from('integration_messages')
        .update({
      ...
      fallbackMessagesStore.forEach((m) => {
        if (m.thread_id === threadId && m.shop_id === shopId) {
          m.assigned_to_user_id = assignedToUserId;
          m.assigned_to_name = assignedToName;
        }
      });
    }
    return true;
