...
export class InvoiceService {
  async getInvoiceById(invoiceId: string, shopId: string) {
    ...
    if (error || !invoice) {
      // Fallback: simple select without deep join in case foreign key relationship isn't indexed by PostgREST
      ...
      if (fallback.error || !fallback.data) {
        throw new Error(error?.message || fallback.error?.message || 'Invoice not found');
      }
      invoice = fallback.data;

      // Manually fetch order items if order_id exists
      if (invoice.order_id) {
    ...
    const itemRows = items.length > 0
      ? items
          ...
          <tr>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${item.products?.product_name || 'Standard Product'}</td>
            ...
            <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: right;">PKR ${Number(item.line_total).toLocaleString()}</td>
          </tr>
        `
          )
          .join('')
      : `
        <tr>
          ...
          <td style="padding: 8px; border-bottom: 1px solid #eee;">Invoice Services / Items</td>
          <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: center;">1</td>
          <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: right;">PKR ${Number(invoice.total_amount || 0).toLocaleString()}</td>
          <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: right;">PKR ${Number(invoice.total_amount || 0).toLocaleString()}</td>
        </tr>
      `;
