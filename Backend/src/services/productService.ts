...
export class ProductService {
  ...
  async getStockByProductId(productId: string, shopIdInput: string) {
    const safeShopId = toSafeUUID(shopIdInput);
    const { data: stock, error } = await supabaseAdmin
      .from('stocks')
      .select('*')
      .eq('product_id', productId)
      .in('shop_id', [shopIdInput, safeShopId])
      .maybeSingle();

    if (error || !stock) throw new Error(error?.message || 'Stock record not found');
    const available_quantity = Math.max(0, (stock.total_quantity || 0) - (stock.reserved_quantity || 0));
    return {
      ...stock,
      available_quantity,
