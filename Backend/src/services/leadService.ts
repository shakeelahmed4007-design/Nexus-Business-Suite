...
export class LeadService {
  ...
  async createLead(userId: string, shopId: string, data: CreateLeadRequest): Promise<Lead> {
    ...
    const safeAssigneeId = toSafeUUID(data.assignedToUserId || userId);
    ...
    if (cleanEmail || formattedPhone) {
      ...
      if (existing) {
        throw new Error('A lead with this email or phone number already exists in this shop');
      }
    }
    ...
    const { data: newLead, error } = await supabaseAdmin
      ...
      .insert({
        ...
        assigned_to_user_id: safeAssigneeId,
        notes: data.notes,
  ...
  async getLeads(
    ...
    if (query.assignedTo) dbQuery = dbQuery.eq('assigned_to_user_id', toSafeUUID(query.assignedTo));
  ...
  async getLead(userId: string, shopId: string, leadId: string, userRole?: string): Promise<any> {
    const safeShopId = toSafeUUID(shopId);
    let dbQuery = supabaseAdmin
      .from('leads')
      .select('*')
    ...
    const { data: lead, error } = await dbQuery.single();
  ...
  async updateLeadStatus(userId: string, shopId: string, leadId: string, newStatus: string): Promise<Lead> {
    ...
    // If status became Won or Converted, create an auto follow-up task
    if ((newStatus === 'Won' || newStatus === 'Converted') && existing.leadStatus !== newStatus) {
      try {
        const dueDate = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000); // 1 day
        await supabaseAdmin.from('tasks').insert({
          shop_id: shopId,
          created_by_id: userId,
          title: `Sales Deal Won - Finalize Agreement & Onboarding: ${existing.firstName} ${existing.lastName}`,
          description: `Deal closed won for lead ${existing.firstName} ${existing.lastName} (${existing.companyName || 'Company'}). Prepare final order/invoice and schedule onboarding.`,
          ...
          linked_entity: 'lead',
          linked_entity_value: leadId,
          assigned_to_user_id: existing.assignedToUserId || userId,
          remind_before_minutes: 30,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        });
      } catch (err) {
        console.warn('Auto task for won lead notice:', err);
      }
    }
