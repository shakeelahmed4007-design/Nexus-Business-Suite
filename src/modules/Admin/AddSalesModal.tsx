...
export interface SalesMemberInput {
  firstName: string;
  lastName: string;
  email: string;
  phone: string;
...
export function AddSalesModal({ isOpen, onClose, onSuccess }: AddSalesModalProps) {
  ...
  const [salesTerritory, setSalesTerritory] = useState('Central Region');
  ...
  const [password, setPassword] = useState('Nexus#2026!Sales');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setEmail('');
      setPhone('');
      setSalesTerritory('Central Region');
  ...
  }, [firstName, lastName, email, phone, salesTerritory, commissionRate, shopAssignments, status, password, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to create sales account.');
      setIsSubmitting(false);
    }
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-ink-500 dark:text-ink-400">
                  Super Admin Console — Create sales rep accounts, set territories, & assign commission rates
                </p>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[78vh] overflow-y-auto scrollbar-thin">
                ...
                <div className="flex items-center justify-between border-b border-ink-200/60 pb-2.5 dark:border-ink-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-ink-900 dark:text-ink-50">
                      Nexus AI Sales Member Validator
                    </span>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="email"
                    required
                    placeholder="hassan@nexusglobal.com"
                    value={email}
            ...
            {/* Sales Territory & Commission Rate */}
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs font-semibold text-ink-700 dark:text-ink-200">
                  Sales Territory
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="South Territory">South Territory</option>
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-ink-100 dark:border-ink-800">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-brand-600 via-brand-500 to-accent-500 px-5 py-2 text-xs font-semibold text-white shadow-md hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
              >
                <Briefcase className="h-4 w-4" />
                {isSubmitting ? 'Creating Sales Account...' : 'Create Sales Member'}
              </button>
            </div>
          </form>
        </motion.div>
