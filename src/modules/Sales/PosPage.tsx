...
import { Search, ShoppingCart, Plus, Minus, Receipt, X, CreditCard, Banknote } from 'lucide-react';
...
import { products, productCategories, type Product } from '@/modules/Sales/products';
...
export function PosPage() {
  ...
  const displayProducts = hasAccess ? products : [];
...
function ReceiptModal({ open, onClose, onComplete, cart, subtotal, tax, total }: { open: boolean; onClose: () => void; onComplete: () => void; cart: CartItem[]; subtotal: number; tax: number; total: number }) {
  return (
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="mb-4 border-t border-dashed border-ink-200 pt-4 dark:border-ink-700">
                {cart.map((item) => (
              ...
              <div className="mt-6 flex gap-2">
                ...
                <Button className="flex-1" onClick={onComplete}>
                  <Banknote className="h-4 w-4" /> Complete
                </Button>
              </div>
            </div>
          </motion.div>
