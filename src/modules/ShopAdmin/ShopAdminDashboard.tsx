    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6">
      <div className="max-w-md w-full text-center space-y-6 bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl">
        ...
        <div className="bg-slate-950/60 p-4 rounded-xl text-left border border-slate-800 space-y-2 text-xs text-slate-300">
          <div><span className="text-slate-500 font-medium">Role:</span> <span className="text-blue-400 font-semibold uppercase">{profile?.role}</span></div>
          <div><span className="text-slate-500 font-medium">Shop ID:</span> <span className="font-mono text-slate-300">{shopId || 'N/A'}</span></div>
          <div><span className="text-slate-500 font-medium">Status:</span> <span className="text-emerald-400 font-medium">Tenant Isolated (RLS Active)</span></div>
        </div>

        <div className="border-t border-slate-800 pt-6">
          <p className="text-sm text-slate-400 mb-4">Shop Admin Module Dashboard coming soon!</p>
          <button
            onClick={() => signOut()}
